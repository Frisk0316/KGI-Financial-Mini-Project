...
3. 透過本地規則式引擎自動將長篇文件拆解為 **2 分鐘學習微模組（Sprint）**
4. 以左右分割畫面預覽原文與系統生成結果
...
系統不依賴任何生成式 AI 或外部 API，而是使用本地規則式流程：

1. 清理文件內容並依段落、句子切分
2. 依內容長度組成約 1 至 3 分鐘的學習模組
3. 以首句與所選 domain 自動產生標題、摘要與重點提醒
4. 將結果存回資料庫，保留文件與 domain 的關聯
...
3. **生成微模組** — 點擊「生成學習微模組」按鈕，系統將依段落與句子規則自動拆解內容

生成完成後，畫面左側顯示原始文件內容，右側顯示系統生成的 Sprint 卡片；每張卡片包含標題、內容、重點摘要、閱讀時間，以及本次套用的知識領域標籤。
